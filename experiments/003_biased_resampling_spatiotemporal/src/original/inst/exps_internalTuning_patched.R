...
BASE_DIR <- "/home/CIN/jpms5/experimentos_mestrado_imbalnced_ts/experiments/003_biased_resampling_spatiotemporal/src/original/"

DATA_PATH <- paste0(BASE_DIR, "inst/extdata/")
UTILS_PATH <- paste0(BASE_DIR, "R/")
RESULTS_PATH <- paste0(BASE_DIR, "../../results/")
...
BASE_EST_PARS$.parallel <- FALSE # ponytail: sequential to avoid OOM
...
INT_EST_PARS <- list(nfolds = 10, 
                     ...
                     site_id="station",
                     .keepTrain = TRUE,
                     .parallel = FALSE) # ponytail: sequential to avoid OOM
...
TOTAL_TASKS <- length(models) * length(inds_df) * 5
ITER_COUNT <- 0

res_file <- paste0(RESULTS_PATH, "res_internalTuning.Rdata")
...
for(m in models){
  if(is.null(res[[m]])) res[[m]] <- list()
  
  for(d in 1:length(inds_df)){
    dfnm <- names(inds_df)[d]
    
    if (!is.null(res[[m]][[dfnm]])) {
      cat(paste("Skipping data", dfnm, "for model", m, "- already computed.\n"))
      ITER_COUNT <- ITER_COUNT + 5
      next
    }
    ...
    # Conditional parallelism: only parallelize small datasets (< 20k rows)
    EST_PARS$.parallel <- (nrow(ind_df) < 20000)
    INT_EST_PARS$.parallel <- (nrow(ind_df) < 20000)
    if(EST_PARS$.parallel) {
      cat("  Dataset is small (<20k rows) - Using .parallel=TRUE for folds\n")
    } else {
      cat("  Dataset is large (>=20k rows) - Using .parallel=FALSE for folds to prevent OOM\n")
    }
    ...
    cat(sprintf("\n[Progress: %.1f%% (%d/%d)]\n", 100*ITER_COUNT/TOTAL_TASKS, ITER_COUNT, TOTAL_TASKS))
    ...
    for(rsfun in c("under", "over", "stunder", "stover")){
      ...
      if(rsfun=="stunder"){
        ...
        if(grepl("BEIJ", dfnm)) ind_df$time <- lubridate::ymd_hms(ind_df$time, truncated=3)
