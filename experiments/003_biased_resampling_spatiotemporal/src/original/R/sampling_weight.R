...
get_space_wts <- function(df, phi, rel.thr, sites_sf=NULL,
  ...
  for(i in 1:length(unique(timz))){
    ...
    if(!length(relev_inds)){
      ...
      all_sites <- df[inds_t, site_id]
      
      # Vectorized minimum distance to a *different* relevant site
      dists_no_self <- dists
      diag(dists_no_self) <- Inf
      
      D <- dists_no_self[paste0("SITE_", all_sites), paste0("SITE_", relev_sites), drop=FALSE]
      d_vec <- apply(D, 1, min)
      
      # If a site's only relevant neighbor is itself, min will be Inf. Set to max_dist.
      d_vec[d_vec == Inf] <- max_dist
      
      space_wts[inds_t] <- d_vec
    }
