...
get_all_neib_vals <- function(df, max_radius,
                              ...
                              time_id, site_id, parallel=FALSE, nsplits=4, full_df=df){
  ...
  neib_df <- neib_df %>% dplyr::left_join(full_df %>% dplyr::select(one_of(cols)), 
...
get_st_indicator <- function(all_neib_vals, stat, radius, 
  ...
  stat_df <- all_neib_vals %>% 
    dplyr::filter(st_dist <= radius) %>%
    dplyr::group_by(dplyr::across(dplyr::all_of(c(time_id, site_id))))
  if(stat=="weighted.mean"){
    stat_df <- stat_df %>%  
      dplyr::summarize(value=stats::weighted.mean(.data[[var]], w=1/st_dist, na.rm=TRUE), .groups="drop")
  }else{
    fun <- match.fun(stat)
    stat_df <- stat_df %>%  
      dplyr::summarize(value=fun(.data[[var]], na.rm=TRUE), .groups="drop")
  }
  colnames(stat_df)[colnames(stat_df) == "value"] <- ind_name
  stat_df
}
...
get_st_indicators <- function(df, stations_sf, radiuses = c(0.1), 
  ...
  # ponytail: chunk rows to avoid building huge intermediate neighbor lists
  chunk_size <- 5000
  n_rows <- nrow(df)
  n_chunks <- ceiling(n_rows / chunk_size)
  inds_df_list <- list()
  
  for (c_i in 1:n_chunks) {
    start_row <- (c_i - 1) * chunk_size + 1
    end_row <- min(c_i * chunk_size, n_rows)
    chunk_df <- df[start_row:end_row, ]
    
    chunk_inds <- chunk_df[, c(site_id, time_id)]
    ...
    for(radius in radiuses){
      ...
      neib_vals <- get_all_neib_vals(df=chunk_df, max_radius=radius, 
                                     t_dist_mat=new_t_dist_mat, s_dist_mat=s_dist_mat,
                                     alpha=alpha, vars=vars, 
                                     time_id=time_id, site_id=site_id,
                                     parallel=parallel, nsplits=nsplits, full_df=df)
      
      for(stat in stats){
        for(var in vars){
          ind_name <- paste0(var, "_", stat, "_", radius)
          chunk_inds <- chunk_inds %>% dplyr::left_join(get_st_indicator(neib_vals, 
    ...
    inds_df_list[[c_i]] <- chunk_inds
    rm(neib_vals, chunk_inds)
    gc()
  }
  
  inds_df <- dplyr::bind_rows(inds_df_list)
  inds_df
}
