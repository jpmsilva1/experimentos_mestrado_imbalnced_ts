...
estimates <- function(data, form, estimator="kf_xval",
  ...
  # Strip train data from rawRes to prevent huge memory accumulation
  # The data is already consumed by evaluator
  for(i in seq_along(res_raw)){
    res_raw[[i]]$train <- NULL
    res_raw[[i]]$trainCols <- NULL
  }
  
  list(evalRes = est.res, rawRes = res_raw)
}
