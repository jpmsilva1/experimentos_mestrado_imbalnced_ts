...
load("../original/repo/Data/DataSets15.Rdata")
...
w <- "nnet"
foreach(d = seq_along(myDSs), .packages = c('performanceEstimation', 'UBL', 'uba', 'e1071', 'randomForest', 'earth', 'nnet')) %dopar% {
  resObj <- paste(myDSs[[d]]@taskName,w,'Res',sep='')
  assign(resObj,
         try(
           performanceEstimation(
             myDSs[d],
             c(Workflow('WFnone_nnet_s1_d0', learner=w),
               Workflow('WFnone_nnet_s2_d0', learner=w),
               Workflow('WFnone_nnet_s5_d0', learner=w),
               Workflow('WFnone_nnet_s10_d0', learner=w),
               Workflow('WFnone_nnet_s1_d0.01', learner=w),
               Workflow('WFnone_nnet_s2_d0.01', learner=w),
               Workflow('WFnone_nnet_s5_d0.01', learner=w),
               Workflow('WFnone_nnet_s10_d0.01', learner=w),
               do.call('workflowVariants', 
                       c(list('WFRU_nnet_s1_d0',
                              learner=w,
  ...
  if (class(get(resObj)) != 'try-error') save(list=resObj,file=paste('results/NNET/',myDSs[[d]]@taskName,w,'Rdata',sep='.'))
}
