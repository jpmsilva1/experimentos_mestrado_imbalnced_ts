...
pb <- txtProgressBar(min = 0, max = length(myDSs), style = 3)
...
for(d in seq_along(myDSs)) {
  ...
  if (class(get(resObj)) != 'try-error') {
    save(list=resObj,file=paste0('results/LM/', myDSs[[d]]@taskName,'.',w,'.Rdata'))
  } else {
    cat(paste("\\nError on dataset", myDSs[[d]]@taskName, ":\\n"))
    print(get(resObj))
  }
}
close(pb)
